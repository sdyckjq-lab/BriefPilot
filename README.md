...
BriefPilot 把一句模糊的产品页、官网或应用页面需求，变成一套能复制、能验证、能继续打磨的设计包。第一轮先复制 `design-spec.md` 去生成；生成第一版后，再用 `/bp-review` 做结果评审和下一轮修改建议。
...
1. 用 `/bp` 输入一句话需求。
2. 打开输出目录，先读 `START_HERE.md`，再复制整个 `design-spec.md`。
3. 把 `design-spec.md` 交给目标设计生成工具，生成第一版。
4. 用 `/bp-review` 把第一版带回来评审，拿到修改建议或下一轮提示词。

## 它解决什么

很多 AI 设计结果不是在生成时才坏掉，而是在输入阶段就不清楚。直接把一句话交给设计工具，常见结果是泛泛的功能卡片、模糊 CTA、缺少可信证据。BriefPilot 先回答：做给谁看、为什么做、要证明什么、页面该怎么组织、视觉方向是什么、最后怎么判断结果够不够好。
...
agent 会诊断需求、补充假设或提问、选择策略，并保存一套可复用资产。普通用户按 “新手 4 步” 走就够了：先打开 `START_HERE.md`，然后完整复制 `design-spec.md` 到目标设计工具。
